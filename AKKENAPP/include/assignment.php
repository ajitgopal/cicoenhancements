	...
	function displayRatesHistory($assign_id)
	{
		...
		else
		{
			...
			{					
				...
				//$hisdate  =  $his_row['m_date'];
				$table_content .= "<tr class='summaryform-formelement ".$class."'><td style='padding-left: 15px;'><a style='text-decoration:underline;' href='javascript:OpenHistory(\"$hisdate\",\"$his_sno\")'>{$hisdate}</a></td>";
				}
...
getCSSLibs(['jqueryUI','jqueryUIStructure','jqueryUITheme']);?>
...
.JoAssignPerdiemModal-wrapper{position: fixed !important;}
.JoAssignPerdiemModal-wrapper .scroll-area{width: 440px !important;}

.assignPerdiemReAssignModal-wrapper{position: fixed !important; width:440px !important; height: 353px !important;}
...
<body onLoad="hideElements('onload');">
...
<!-- code ends --->
	...
	<input type="hidden" name="symmetry" id="symmetry" value="<?php echo AKKUPAY_WITH_SYMMETRY;?>" />
...
<tr>
	...
	<table width=98% class="customProfile">
	<tr>
	<td>
	<fieldset>
	<div class="form-back">
	...
	<tr>
			...
			<td colspan=3 class="summaryform-formelement">&nbsp;<?php echo html_tls_specialchars($empcandname,ENT_QUOTES);?>
			<td style="display:none;"><select name=empname id=empname class="summaryform-formelement"><option value='<?php echo $empdispname; ?>' selected><?php echo $empdispname; ?></option><?php echo stripslashes($enames); ?></select><input type="hidden" name="selectedEmpsno" id="selectedEmpsno"/></td><td>
		</tr>
	<?php }?>	
	<?php
	if(($modfrom=="newasgmt" || $copyasign =="yes") && !isset($new_user))
	{
		?>
		<tr>
			<td width="167" class="crmsummary-content-title">Select&nbsp;an&nbsp;Employee&nbsp;</td>
	...
	*/ 
	...
	if($mode=="newassign")
	{
		$query2="select contact_manage.city,contact_manage.state,contact_manage.country,hrcon_compen.company_id from contact_manage,hrcon_compen where contact_manage.status !='BP' and hrcon_compen.location=contact_manage.serial_no and hrcon_compen.username='{$new_user}' and hrcon_compen.ustatus='active'";
	}
	else
	{
		/*if($elements[60]!="" && $elements[60]!="0" )
			$query2="select contact_manage.city,contact_manage.state,contact_manage.country,hrcon_compen.company_id from contact_manage,hrcon_compen where contact_manage.status !='BP' and hrcon_compen.location=contact_manage.serial_no and contact_manage.serial_no='{$elements[60]}'";
		else*/
		if($modfrom == 'hiring'){
			$query2="select contact_manage.city,contact_manage.state,contact_manage.country,cc.company_id from contact_manage,consultant_compen cc where contact_manage.status !='BP' and cc.location=contact_manage.serial_no and cc.username='{$conusername}'";
		}else{
			$query2="select contact_manage.city,contact_manage.state,contact_manage.country,hrcon_compen.company_id from contact_manage,hrcon_compen where contact_manage.status !='BP' and hrcon_compen.location=contact_manage.serial_no and hrcon_compen.username='{$conusername}' and hrcon_compen.ustatus='active'";
		}
			
	}
	$res2=mysql_query($query2,$db);
	$dd2=mysql_fetch_row($res2);
	$dd2[3] = ($company_sno) ? $company_sno : $dd2[3];
	...
	if(PAYROLL_PROCESS_BY_MADISON == "MADISON" && ($modfrom == "employee" || $modfrom == "newasgmt" || $modfrom == "approve" || $modfrom == "updateasgmt" || $modfrom == "empman")) 
	{
		...
		<tr>
			...
			<td><input class="form-control w-250" type=text name="madisonorderid" id="madisonorderid" size=25 maxsize=150 maxlength=150 value="<?php echo html_tls_specialchars($elements[80],ENT_QUOTES);?>"></td>
		</tr>
		<?php
	}
	else 
	...
	<tr>
		...
		<td>
		    ...
		    {
		    ?>			
			    <input class="form-control w-250" type=text name=jotitle size=45 maxsize=150 maxlength=150 value="<?php echo html_tls_specialchars($elements[4],ENT_QUOTES);?>" setName='Assignment Name' <?php echo $spl_Attribute?>>&nbsp;<?php if($jobPosVal!="" && $jobPosVal!=0) {?><a href="javascript:viewSummary('job','<?php echo $jobPosVal?>');" class="edit-list">view job order</a><?}?>
			    <?php
			}
			    ...
			    {
			    ?>			
			    	<input class="summaryform-formelement form-control w-250" type=text name=jotitle size=45 maxsize=150 maxlength=150 value="<?php echo html_tls_specialchars($elements[4],ENT_QUOTES);?>" setName='Assignment Name' <?php echo $spl_Attribute?>>&nbsp;<?php if($jobPosVal!="" && $jobPosVal!=0) {?><a href="javascript:viewSummary('job','<?php echo $jobPosVal?>');" class="edit-list">view job order</a><?}?>
			    <?php
			    }
	...
	if($modfrom=="empman" || $modfrom=="employee" || $modfrom=="approve" || $modfrom=="newasgmt" || $modfrom == "updateasgmt" || $copyasign =="yes")
	{
		...
		<tr id="assignment_status">
			...
			<td>
				<select class="w-250 custom-select" name=astatus id=astatus <?php echo $assg_disable;?> onChange="dispCancelNotes(this);">
				<?php 
				if($copyasign == 'yes')
				{
					?>
	...
	<tr id="cancelReason" style="display:none">
		...
		<td colspan=3 class="summaryform-formelement">&nbsp;<select class="form-select w-250" name="cancel_reason" id="cancel_reason" ><option value="0">Select Reason</option></select>&nbsp;</td>
	...
	<tr id="closeReason" style="display:none">
		...
		<td colspan=3 class="summaryform-formelement">&nbsp;<select class="form-select w-250" name="close_reason" id="close_reason" ><option value="0">Select Reason</option></select>&nbsp;</td>
	...
	<tr id="cancel_notes" style="display:none">
		...
		<td><div class='d-flex align-items-center gap-2'><textarea name="notes_cancel" cols="32" class="form-control w-250" rows="3" id="notes_cancel"></textarea>&nbsp;Please type a reason for cancelling this assignment</div></td>
	</tr>
	</table>
	</div>
	...
	<div id="jobType1" style="<?php echo $jtype_style;?>">
		...
		<tr>
			...
			<td>
				<select class="form-select w-250" name="jotype" id="jotype" onChange="hideElements('jotypechange');" <?php echo $job_type_disable;?>>
				...
				$place_jobtype="";
				$joclassification ="";
				$que1="select sno,name,type from manage where type='jotype' or type='jocategory' or type='joindustry' or type='joclassification' order by name";
				$res1=mysql_query($que1,$db);				
				while($dd1=mysql_fetch_row($res1))
				{
					...
					{
						$joclassification.="<option  value='".stripslashes($dd1[0])."'".sele($classification,$dd1[0])." >".stripslashes($dd1[1])."</option>";
					}
				}
				?>
				</select>
		...
		<tr>
			...
			<td align=left id="dynamicDepts"><?php departmentSelBox('deptassignment', $elements[88], 'summaryform-formelement form-select w-250','','','','','',$dd2[3]);?></td>
		</tr>
		</table>
		</div>
	</div>
	...
	<div id="jobDetails1" style="<?php echo $jobdet_style;?>">
		...
		<tr>
			...
			<td align=left>
				<select class="summaryform-formelement form-select w-250" name="jocat">
				<option value="">--select--</option>
				<?php echo $jocat; ?>
				</select>
		...
		<tr>
			...
			<td align=left><input class="summaryform-formelement form-control w-250" type=text name=jorefcode size=40 maxsize=25 maxlength=25 value="<?php echo html_tls_specialchars($elements[5],ENT_QUOTES);?>"></td>
		</tr>
		
		<tr>
			<td width="167" class="crmsummary-content-title">CORP CODE</td>
			<td align=left>
				<select name="corp_code" id="corp_code" class="summaryform-formelement form-select w-250" style="overflow:auto;<? echo $wid_val; ?>">
		...
		<tr>
			<td width="167" class="crmsummary-content-title">Ultigigs Timesheet Layout</td>
			<td align=left>
				<select name="assignment_timesheet_layout_preference" id="assignment_timesheet_layout_preference">
					<option value="" <?php if($assignment_timesheet_layout_preference == "") { echo "selected"; } ?>>--- Select Template ---</option>
					<option value="Regular" <?php if($assignment_timesheet_layout_preference == "Regular") { echo "selected"; } ?>>Regular</option>
					<option value="TimeInTimeOut" <?php if($assignment_timesheet_layout_preference == "TimeInTimeOut") { echo "selected"; } ?>>Time In &amp; Time Out</option>
					<option value="Clockinout" <?php if($assignment_timesheet_layout_preference == "Clockinout") { echo "selected"; } ?>>Clock In &amp; Out</option>
				</select>
			</td>
		</tr>
		...
		if(AKKUPAY_WITH_SYMMETRY == 'Y'){
			    if($worksitecode!=''){
					$getworkstate = "SELECT state FROM contact_manage WHERE loccode = '".$worksitecode."'"; 
					$getworkstates = mysql_query($getworkstate,$db);
					$resworkstates = mysql_fetch_row($getworkstates);
					if($resworkstates[0]=='WA')
					{
						$gettitleTimesheetCounts = "SELECT COUNT(1) FROM timesheet_hours WHERE assid = '".$showAssignid."' AND status NOT IN ('Backup','Deleted')";
						$restitleTimesheetCounts = mysql_query($gettitleTimesheetCounts,$db);
						$rowtitleTimesheetCounts = mysql_fetch_array($restitleTimesheetCounts);
						$worklicode = "";
						if($rowtitleTimesheetCounts[0] > 0)
						{
							$worklicode = "Disabled";
						}
					}
			...
			if($modfrom=="hiring"){ 
			$workSiteQuery = "SELECT cm.serial_no,cm.address,
								   cm.city,
								   cm.state,
								   cm.zipcode,
								   cm.loccode
							  FROM contact_manage cm
							  LEFT JOIN company_setup cs ON cs.sno = cm.comp_sno
							 WHERE cm.comp_sno = '".$compenTabvalues[59]."' AND cm.status!='BP' AND cm.inactive!='Y' AND cs.inactive!='Y' order by cm.serial_no ";
                     
                    $Response = mysql_query($workSiteQuery, $db);
					while ($Row = mysql_fetch_row($Response)) 
					{
							$locaddr = explode("|",$Row[1]); 
							$worksitedrop = $Row[5].'-'.$locaddr[0].','.$locaddr[1].','.$Row[2].','.$Row[3].','.$Row[4];
							if($Row[5]==$worksitecode){ 
							$workSite_option.='<option  value="'. $Row[5] .'" selected>'. $worksitedrop . '</option>'; 
							} else{
							$workSite_option.='<option  value="'. $Row[5] .'">'. $worksitedrop . '</option>';
							}
					}?>
			<tr>
						...
						<div class="col-auto">
					<select class="form-select w-250"  name="worksitecode" id="worksitecode" onChange="javascript:showwali()" <?=$worklicode?>>
					<option value=""> -- Select -- </option>
			...
			<?php if(DEFAULT_AKKUPAY == 'Y'){ ?> 
				...
				<?php 
				  if($modfrom=="hiring" && AKKUPAY_WITH_SYMMETRY != 'Y'){ 
					...
					<div class="row align-items-center">
					<div class="col-auto">
						<select class="form-select w-250"  name="worksitecode" id="worksitecode">
							<option value=""> -- Select -- </option>
							<?php echo $workSite_option;?>
						</select>
                    </div>
                  ...
                  <?php } else {
					  ...
					  if(!empty($candID) || $modfrom=="newasgmt"){ ?>
						...
						<td>
							...
							<?php  
							if(AKKUPAY_WITH_SYMMETRY == 'Y'){
								$workSiteCode = getCompworkSiteCodes($worksitecode,$candID);
											echo $workSiteCode;
							}elseif(DEFAULT_AKKUPAY=='Y' && AKKUPAY_WITH_SYMMETRY != 'Y'){
							$workSiteCode = getworkSiteCodes($worksitecode,$candID);
											echo $workSiteCode;
							}
							?>
							</select>
            ...
            <?php }
					 ...
					 if(AKKUPAY_WITH_SYMMETRY == 'Y'){
					 	...
					 	if($daypay=='Yes'){
					 		$chk_flg = 'checked';
					 		$read_only = '';
					 		$mphrQuery = "SELECT count(*) FROM mpHR_payData WHERE AssignmentID = '".$showAssignid."' ";
	                        $mphrQueryRes = mysql_query($mphrQuery, $db);
	                        $mphrQueryRow = mysql_fetch_row($mphrQueryRes);
	                       /* if($mphrQueryRow[0]>0){
	                        	$read_only = 'disabled';
	                        	$disabled_text = "style='opacity: 1.0' ";
	                        }*/
	                         if(($rowTimesheetCount[0] > 0 || $rowExpenseCount[0] > 0 || $rowPlacementCount[0] >0) && $copyasign !='yes'){
	                        	$read_only = 'disabled';
	                        	$disabled_text = "style='opacity: 1.0' ";
						...
						$certified_block = "style='display:none'";
						if($federal_payroll=='Yes'){
					 		$fed_flag = 'checked';
					 		$fedread_only = '';
					 		$feddisabled_text = "";
					 		$fedmphrQuery = "SELECT count(*) FROM mpHR_payData WHERE AssignmentID = '".$showAssignid."' ";
	                        $mphrQueryRes = mysql_query($fedmphrQuery, $db);
	                        $fedmphrQueryRow = mysql_fetch_row($mphrQueryRes);

	                        if(($rowTimesheetCount[0] > 0 || $rowExpenseCount[0] > 0 || $rowPlacementCount[0] >0) && $copyasign !='yes'){
	                        	$fedread_only = 'disabled';
	                        	$feddisabled_text = "style='opacity: 1.0' ";
	                        }

	                        /*if($fedmphrQueryRow[0]>0){
	                        	$fedread_only = 'disabled';
	                        	$feddisabled_text = "style='opacity: 1.0' ";

	                        }*/
	                        //$certified_block = "style='display:block'";
	                        //echo '123';
						...
						if($nonfederal_payroll=='Yes'){
					 		...
					 		$nonfedread_only = '';
					 		$feddisabled_text = "";
					 		$nonfedmphrQuery = "SELECT count(*) FROM mpHR_payData WHERE AssignmentID = '".$showAssignid."' ";
	                        $mphrQueryRes = mysql_query($nonfedmphrQuery, $db);
	                        $nonfedmphrQueryRow = mysql_fetch_row($mphrQueryRes);

	                        if(($rowTimesheetCount[0] > 0 || $rowExpenseCount[0] > 0 || $rowPlacementCount[0] >0) && $copyasign !='yes'){
	                        	$nonfedread_only = 'disabled';
	                        	$nonfeddisabled_text = "style='opacity: 1.0' ";
						...
						if(($rowTimesheetCount[0] > 0 || $rowExpenseCount[0] > 0 || $rowPlacementCount[0] >0) && $copyasign !='yes'){
	                        	$certifiedread_only = 'disabled';
	                        	$certified_id= "id='disabled'";
						...
						<script type="text/javascript">
					        $(function () {
					        	...
					        	if(assign=="edit"){
					        		if($('#nonfederal_payroll').is(':checked') || $('#federal_payroll').is(':checked')){
					        			$('#certified').val('Y');
					        		}else{
					        			$('#certified').val('N');
					        		}
								...
								var tcnt= "<?php echo $rowTimesheetCount[0];?>";
								var expcnt = '<?php echo $rowExpenseCount[0];?>';
								var plccnt = '<?php echo $rowPlacementCount[0];?>';
								var copyasign = '<?php echo $copyasign;?>';
								var wcval = $('#joclassification').val();
						        	var pcodeval = $('#projcode').val();
						       		if(((tcnt > 0 || expcnt > 0 || plccnt >0) && copyasign !='yes') && wcval!='' &&  pcodeval!=''){
									$("#projcode" ).prop( "disabled", true );
									$("#joclassification" ).prop( "disabled", true );
									$("#wcedit").removeAttr("href");
								}else{
									$( "#projcode" ).prop( "disabled", false );
									$( "#joclassification" ).prop( "disabled", false );
								}
						...
						<div class="form-check">
							<input class="" type=checkbox name="daypay" id = "daypay" <?php echo $chk_flg;?> <?php echo $read_only;?> >
							<label class="form-check-label" for="daypay" <?php echo $disabled_text; ?> >Daily Pay Assignment</label>
						</div>
						</div>

						<div class="col-auto">
						<div class="form-check">
							<input class="" type=checkbox name="nonfederal_payroll" id = "nonfederal_payroll" <?php echo $nonfed_flag;?> <?php echo $nonfedread_only;?> >
							<label class="form-check-label" for="nonfederal_payroll" class="certifiednonfed" <?php echo $nonfeddisabled_text; ?> >Certified Payroll - Non Federal</label>
						</div>
						</div>

						<div class="col-auto">
						<div class="form-check">
							<input class="" type=checkbox name="federal_payroll" id = "federal_payroll" <?php echo $fed_flag;?> <?php echo $fedread_only;?> >
							<label class="form-check-label"  for="federal_payroll" class="certifiedfed" <?php echo $feddisabled_text; ?> >Certified Payroll - Federal</label>
						</div>
						</div>
			...
			<tr id="colprojcode" <?php echo $certified_block;?>>
				...
				<td>
					<select class="form-select w-250" id="projcode" name="projcode">
						<option value="">--Select--</option>
						<?php
						...
						if($modfrom == 'hiring'){
							$cc_query	= "SELECT cp.sno,cp.project_code FROM certified_payroll cp 
										...
										WHERE cm.serial_no='".$compenLocationId."' AND cp.inactive!='Y'
										";
							$cc_result	= mysql_query($cc_query, $db);
							...
							while ($cc_data = mysql_fetch_row($cc_result)) {
								...
								echo "<option value='$cc_data[0]' ".sele($cc_data[0], $contractid)." title='".html_tls_specialchars($cc_text, ENT_QUOTES)."'>".html_tls_specialchars($cc_text, ENT_QUOTES).'</option>';
							}
						...
						if($new_user!=''){
							$conusername = $new_user;
						}else{
							$conusername = $elements[1];
						}
						$cc_query	= "SELECT cp.sno,cp.project_code FROM certified_payroll cp 
										LEFT JOIN contact_manage cm ON (cp.location_code = cm.loccode) AND cm.status!='BP'
										LEFT JOIN emp_list e ON (cm.serial_no = e.emp_locid)
										WHERE e.username='".$conusername."' AND cp.inactive!='Y'
										";
						$cc_result	= mysql_query($cc_query, $db);
			...
			<tr id="colclassification" <?php echo $certified_block;?>>
				...
				<td align=left>
					<select class="summaryform-formelement form-select w-250" name="joclassification" id="joclassification">
					<option value="">--Select--</option>
					<?php echo $joclassification; ?>
					</select>
					<a href="javascript:doManage('joclassification','joclassification');" class="edit-list" id="wcedit" >edit list</a>
				</td>
			</tr>
			<?php } ?>		
			...
			<tr id="colwali">
				...
				<td>
					<select class="form-select w-250" id="wali" name="wali" <?=$worklicode?>>
					
					</select>
				</td>
			</tr>
			<?php }else{ ?> 
				<input type="hidden" name="waclassli" id="waclassli" value="">
				<tr id="colwali" style="display:none;"> <td colspan="1"></td> </tr>
			<?php } ?>
		</table>
		</div>
		...
		<table width="100%" class="crmsummary-edit-table align-middle" id="crm-joborder-scheduleDiv-Table">
		<tr class="d-flex align-items-center">
			<td width="167" class="crmsummary-content-title w-100">
		...
		<div class="form-back accordion-item">
		<table width="100%" border="0" class="crmsummary-edit-table accordion-header" id="crm-joborder-billinginfoDiv-Table">
		<tr class="d-flex align-items-center">
			<td width="167" class="crmsummary-content-title w-100">
			...
			<td>
				...
				<div class="form-opcl-btnleftside"><div align="left"></div></div>
				<div id="crm-joborder-billinginfoDiv-plus" class='DisplayNone'><a onClick="classToggle('billinginfo','plus')" class="form-op-txtlnk link-success fs-6 fw-bold" href="#crm-joborder-billinginfoDiv-minus"><b>+</b></a></div>
				<div id="crm-joborder-billinginfoDiv-minus"><a onClick="classToggle('billinginfo','minus')" class="form-cl-txtlnk link-success fs-6 fw-bold " href="#crm-joborder-billinginfoDiv-plus"><b>-</b></a></div>
				<div class="form-opcl-btnrightside"><div align="left"></div></div>
				</span>
			</td>
		</tr>
		...
		<div class="jocomp-back accordion-body" id="crm-joborder-billinginfoDiv" name="crm-joborder-billinginfoDiv">
			<span id=tab_billinginfo style="display:">
				<table width="100%" border="0" class="crmsummary-jocomp-table mb-0">
					<tr>
                        <th colspan="2"><span id=rate_cond>Rates</span></th>
                    </tr>
				</table>
			</span>
		...
		<table width="100%" border="0" class="crmsummary-jocomp-table align-middle mb-0">		
					...
					<tr>
						...
						<td>
							<div class="row align-items-center g-2">
								<?php if(RATE_CALCULATOR=='Y'){ ?>
								<div id="payrate_calculator" style="cursor:pointer;display:none;" class="col-auto">
								...
								<div class="col-auto">
									<div class="form-check">
										<input name="payratetype" id="payratetype" type="radio" onClick="javascript:calrate()" value="rate" <?php echo $payStat;?> <?php echo sent_check("rate",$payFinal[1]);?> class="form-check-input">
								...
								<div class="col-auto">
									...
									<input name="comm_payrate" type="text" id="comm_payrate" value="<?php echo html_tls_specialchars($payRateVal,ENT_QUOTES);?>" size=10 maxlength="9" class="form-control" onfocus="javascript:confirmPayRateAutoCalculatoin('comm_payrate','calculateComPayRates');" <?php if(RATE_CALCULATOR=='N'){ ?> onChange="promptOnChangePayRate();" <?php } ?> onblur="javascript:clearConfirmPayRateAutoCalculatoin();" onkeyup="javascript:calculateComPayRates();" onkeypress="return blockNonNumbers(this, event, true, false);">
								</div>
								<div class="col-auto">
								...
								<div class="col-auto" id='reg_pay_Bill_NonBill'>
									<div class="form-check form-check-inline">
										<input name="payrateBillOpt" id="payrateBillOpt"  type="radio" value="Y" <?php if($ratesDefaultVal['Regular'][0]!=""){ echo getChk($ratesDefaultVal['Regular'][0],"Y");}else{if($displaytypes[0]['value'] == 'B'){echo 'checked="checked"';}} ?> class="form-check-input BillableRates">
										<label class="form-check-label">Billable</label>
									</div>
									<div class="form-check form-check-inline">
										<input name="payrateBillOpt" id="payrateBillOpt"  type="radio" value="N" <?php if($ratesDefaultVal['Regular'][0]!=""){ echo getChk($ratesDefaultVal['Regular'][0],"N");}else{if($displaytypes[0]['value'] == 'NB'){echo 'checked="checked"';}} ?>  class="form-check-input BillableRates">
							...
							<div class="form-check mt-2">
								<span class="d-flex align-items-center gap-2">
									<input name="payratetype" id="payratetype" type="radio" value="open" <?php echo sent_check("open",$payFinal[1]);?> onClick="javascript:calrate()" class="form-check-input">
									<label class="form-check-label">
										Open
										<input name="comm_open_payrate" type="text" id="comm_open_payrate" value="<?php echo html_tls_specialchars($payFinal[3],ENT_QUOTES);?>" size=38 class="form-control form-control-sm">
									</label>
								</span>
							</div>
                            ...
                            <div class="row">
                                <div class="col-auto">
									<div class="input-group">
										<input class="form-control" type="text" name="comm_burden" id="comm_burden" value="<?php if(isset($elements[26])){echo $elements[26]; }else { echo 0; } ?>" <?php if(RATE_CALCULATOR=='Y'){?> onkeyup="doNoCalculateMarkup(this);calculatebtmargin();" <?php } else{ ?> onkeyup="calculatebtmargin();" <?php } ?>  maxlength="9" size="10" onkeypress="return blockNonNumbers(this, event, true, false);"/>
			...
			<tr>
				...
				<td>
					<div class="row align-items-center g-2">
						...
						<?php if(RATE_CALCULATOR=='Y'){ ?>
							<div id="billrate_calculator" style="cursor:pointer;display:none;" class="col-auto">
								<i class="fa fa-calculator" onclick="javascript:billrateCalculatorFunc();" aria-hidden="true"></i>
							</div>
						<?php } ?>
						
					<div class="col-auto">
						<div class="form-check">
							<input type="radio" <?php echo sent_check("rate",$payFinal[2]);?> <?php echo $billStat;?> name="billratetype" id="billratetype" value="rate" onClick="javascript:calrate()" class="summaryform-formelement">
							<label class="form-check-label">Rate</label>
						</div>
					</div>
				
				<!--<span class="summaryform-formelement"><span id="leftflt"><input name="billratevall1" type="text" id="billratevall1" value="<?php echo html_tls_specialchars($billRateVal,ENT_QUOTES);?>" size=10 onBlur="javascript:cal('add','br')">-->
				...
				<div class="col-auto" id='reg_bill_Tax_NonTax'>
					<div class="form-check form-check-inline">
						<input class="form-check-input" name="billrateTaxOpt" id="billrateTaxOpt"  type="radio" value="Y" <?php if($ratesDefaultVal['Regular'][1]!=""){ echo getChk($ratesDefaultVal['Regular'][1],"Y");}else{if($displaytypes[1]['value'] == 'T'){echo 'checked="checked"';}} ?> >
						<label class="form-check-label">Taxable</label>
					</div>
					<div class="form-check form-check-inline">
						<input class="form-check-input" name="billrateTaxOpt" id="billrateTaxOpt"  type="radio" value="N" <?php if($ratesDefaultVal['Regular'][1]!=""){ echo getChk($ratesDefaultVal['Regular'][1],"N");}else{if($displaytypes[1]['value'] == 'NT'){echo 'checked="checked"';}} ?> >
				...
				<div class="form-check">
					<span class="d-flex align-items-center gap-2">
						<input name="billratetype" id="billratetype" type="radio" onClick="javascript:calrate()" value="open" <?php echo sent_check("open",$payFinal[2]);?> class="form-check-input">
						<label class="form-check-label">
							Open
							<input name="comm_open_billrate" type="text" id="comm_open_billrate" value="<?php echo html_tls_specialchars($payFinal[4],ENT_QUOTES);?>" size=38 class="form-control form-control-sm">
						</label>
					</span>
				</div>
			</td>
		...
		<tr id="burden-rate" >
                    ...
                    <td>
                        ...
                        <?php 
                               ...
                               ?>
							<div class="BTContainerd row">
                                <div class="col-auto">   
									<div class="input-group">
										<input type="text" class="form-control" name="comm_bill_burden" id="comm_bill_burden" value="<?php if(isset($elements[97])) { if(isset($elements[92])){echo $elements[92]; }else { echo 0; } }else{ if(isset($elements[93])){echo $elements[93]; }else { echo 0; } }?>" maxlength="9" size="10" <?php if(RATE_CALCULATOR=='Y'){?> onkeyup="doNoCalculateMarkup(this);calculatebillbtmargin();"<?php } else{ ?> onkeyup="calculatebillbtmargin();" <?php } ?>  onkeypress="return blockNonNumbers(this, event, true, false);"/>
										<span class="input-group-text">%</span>
									</div>
								</div>
                            </div>
		...
		<tr id="bill_info_overtimepay" style=" <?php echo $Dir_Int_pay;?>">
                    ...
                    <td>
						...
						<div class="col-auto" id='ot_pay_Bill_NonBill'>
									<div class="form-check form-check-inline">
										<input name="OvpayrateBillOpt" id="OvpayrateBillOpt"  type="radio" value="Y" <?php if($ratesDefaultVal['OverTime'][0] == ''){ if($arr['poption'] == "B"){ echo ' checked="checked"'; echo 'class="'.$penabled_user_input_radio_button.' BillableRates form-check-input"'; }else { echo $pdisable; echo 'class="'.$pdisabled_user_input_radio_button.' BillableRates form-check-input"';} } else if($ratesDefaultVal['OverTime'][0] == 'Y'){echo ' checked="checked"'; echo 'class="'.$penabled_user_input_radio_button.' BillableRates form-check-input"';}else{ echo $pdisable; echo 'class="'.$pdisabled_user_input_radio_button.' BillableRates form-check-input"';}?>>
										<label class="form-check-label">Billable</label>
									</div>
									<div class="form-check form-check-inline">
										<input name="OvpayrateBillOpt" id="OvpayrateBillOpt"  type="radio" value="N" <?php if($ratesDefaultVal['OverTime'][0] == ''){ if($arr['poption'] == "NB"){ echo ' checked="checked"'; echo 'class="'.$penabled_user_input_radio_button.' BillableRates form-check-input"'; }else { echo $pdisable; echo 'class="'.$pdisabled_user_input_radio_button.' BillableRates form-check-input"';} } else if($ratesDefaultVal['OverTime'][0] == 'N'){echo ' checked="checked"'; echo 'class="'.$penabled_user_input_radio_button.' BillableRates form-check-input"';}else{ echo $pdisable; echo 'class="'.$pdisabled_user_input_radio_button.' BillableRates form-check-input"';}?>>
										<label class="form-check-label">Non-Billable</label>
									</div>
						</div>
						</div>
		...
		<tr id="bill_info_overtimebill" style=" <?php echo $Dir_Int_bill;?>">
			...
			<div class="col-auto" id='ot_bill_Tax_NonTax'>
				<div class="form-check form-check-inline">
                        <input name="OvbillrateTaxOpt" id="OvbillrateTaxOpt" class="form-check-input" type="radio" value="Y" <?php if($ratesDefaultVal['OverTime'][1] == ''){ if($arr['boption'] == "T"){ echo ' checked="checked"'; }else { echo $bdisable; } } else if($ratesDefaultVal['OverTime'][1] == 'Y'){echo ' checked="checked"';}else{ echo $bdisable;}?>>
						<label class="form-check-label">Taxable</label>
				</div>
						
				<div class="form-check form-check-inline">		
                        <input name="OvbillrateTaxOpt" id="OvbillrateTaxOpt" class="form-check-input" type="radio" value="N" <?php if($ratesDefaultVal['OverTime'][1] == ''){ if($arr['boption'] == "NT"){ echo ' checked="checked"'; }else { echo $bdisable; } } else if($ratesDefaultVal['OverTime'][1] == 'N'){echo ' checked="checked"';}else{ echo $bdisable;}?>>
						<label class="form-check-label">Non-Taxable</label>
				</div>
			</div>
			</div>
		...
		<tr id="db_time_payrate" >
                    ...
                    <td>
						<div class="row align-items-center g-2">
                        <div class="col-auto">
							<input name="db_time_pay" type="text" id="db_time_pay" value="<?php if($elements[61] == '0.00') echo ""; else echo $elements[61];?>" size=10 maxlength="9" class="summaryform-formelement<?php echo $pdisabled_user_input_field;?>" <?php echo $pdisable;?> onkeypress="return blockNonNumbers(this, event, true, false);"><input type="hidden" id="db_time_pay_hidden" value="<?php echo $arr['pvalue'];?>"></div>
                           ...
                           <div class="col-auto">
							...
							<div class="col-auto" id='dt_pay_Bill_NonBill'>
									<div class="form-check form-check-inline">
										<input name="DbpayrateBillOpt" id="DbpayrateBillOpt"  type="radio" value="Y" <?php if($ratesDefaultVal['DoubleTime'][0] == ''){  if($arr['poption'] == "B"){ echo ' checked="checked"'; echo 'class="'.$penabled_user_input_radio_button.' BillableRates form-check-input"';}else { echo $pdisable; echo 'class="'.$pdisabled_user_input_radio_button.' BillableRates form-check-input"';} }else if($ratesDefaultVal['DoubleTime'][0] == 'Y'){echo ' checked="checked"'; echo 'class="'.$penabled_user_input_radio_button.' BillableRates form-check-input"';}else{ echo $pdisable; echo 'class="'.$pdisabled_user_input_radio_button.' BillableRates form-check-input"';}?>>
										<label class="form-check-label">Billable</label>
									</div>
										
									<div class="form-check form-check-inline">
									   <input name="DbpayrateBillOpt" id="DbpayrateBillOpt"  type="radio" value="N" <?php if($ratesDefaultVal['DoubleTime'][0] == ''){  if($arr['poption'] == "NB"){ echo ' checked="checked"'; echo 'class="'.$penabled_user_input_radio_button.' BillableRates form-check-input"';}else { echo $pdisable; echo 'class="'.$pdisabled_user_input_radio_button.' BillableRates form-check-input"';} }else if($ratesDefaultVal['DoubleTime'][0] == 'N'){echo ' checked="checked"'; echo 'class="'.$penabled_user_input_radio_button.' BillableRates form-check-input"';}else{ echo $pdisable; echo 'class="'.$pdisabled_user_input_radio_button.' BillableRates form-check-input"';}?>>
									   <label class="form-check-label">Non-Billable</label>
									</div>
							</div>
							<span id='reg_pay_Bill_NonBill'> </span>
						</div>
                    </td>
		</tr>
		...
		<tr id="db_time_billrate" >
			...
			<div class="col-auto" id='db_bill_Tax_NonTax'>
				<div class="form-check form-check-inline">
                        <input name="DbbillrateTaxOpt" id="DbbillrateTaxOpt" class="form-check-input" type="radio" value="Y" <?php if($ratesDefaultVal['DoubleTime'][1] == ''){ if($arr['boption'] == "T"){ echo ' checked="checked"'; }else { echo $bdisable; } }else if($ratesDefaultVal['DoubleTime'][1] == 'Y'){echo ' checked="checked"';}else{ echo $bdisable;}?>>
						<label class="form-check-label">Taxable</label>
				</div>
				<div class="form-check form-check-inline">
                        <input name="DbbillrateTaxOpt" id="DbbillrateTaxOpt" class="form-check-input" type="radio" value="N" <?php if($ratesDefaultVal['DoubleTime'][1] == ''){ if($arr['boption'] == "NT"){ echo ' checked="checked"'; }else { echo $bdisable; } }else if($ratesDefaultVal['DoubleTime'][1] == 'N'){echo ' checked="checked"';}else{ echo $bdisable;}?>>
						<label class="form-check-label">Non-Taxable</label>
				</div>
			</div>
			</div>
		...
		<tr>
	        ...
	        <?php  // Hidding Perdiem when Akkupay with Symmetry OR Only Symmetry is Enabled
				if (AKKUPAY_WITH_SYMMETRY == "Y" || AKKUPAY_WITH_SYMMETRY == "Y") { ?>
					<tr id="billable_block" style="">
					<td colspan="2" align="left">
						<input type="hidden" name="txt_lodging" id="txt_lodging" value="0.00">
						<input type="hidden" name="txt_mie" id="txt_mie" value="0.00">
						<input type="hidden" name="txt_total" id="txt_total" value="0.00">
						<input type="hidden" name="sel_perdiem" id="sel_perdiem" value="HOUR">
						<input type="hidden" name="sel_perdiem2" id="sel_perdiem2" value="USD">
						<input type="radio" name="radio_billabletype" id="radio_billabletype" checked="" value="N" style="visibility: hidden;">
						<input type="radio" name="radio_billabletype" id="radio_billabletype" value="Y" style="visibility: hidden;">
						<input type="radio" name="radio_taxabletype" id="radio_taxabletype" checked="" value="N" style="visibility: hidden;">
						<input type="radio" name="radio_taxabletype" id="radio_taxabletype" value="Y" style="visibility: hidden;">
						<input type="hidden" name="diem_billrate" id="diem_billrate" value="0.00">
					</td></tr>
				<?php }else{ ?>
				<tr id="billable_block" style="">
					<td colspan="2" align="left">
						<table width="97%" border="0" cellpadding="0" cellspacing="0">
							...
							<tr>
								...
								<div class="col-auto">
									<div class="form-check form-check-inline">
										<input class="form-check-input" type="radio" name="radio_taxabletype" value="Y" <?php echo sent_check("Y",$elements[77]);?>>
										<label class="form-check-label">Taxable</label>
									</div>
								
									<div class="form-check form-check-inline">
										<input class="form-check-input" type="radio" name="radio_taxabletype" value="N" <?php echo sent_check("N",$elements[77]);?>>
										<label class="form-check-label">Non-Taxable</label>
									</div>
								</div>
								</div>
							...
							<tr>
								...
								<div class="row align-items-center g-2">
									<div class="col-auto">
										<div id="leftflt" class="form-check form-check-inline">
											<input class="summaryform-formelement" type="radio" name="radio_billabletype" value="Y" <?php echo sent_check("Y",$elements[76]);?> onClick="javascript:showBillDiv(this,'txt_total');" >
											<label class="form-check-label">Billable</label>
										</div>
									</div>
									<?php 
									...
									<div class="col-auto">
										<div align="left" id="leftflt" class="form-check form-check-inline">
											<input class="form-check-input" type="radio" name="radio_billabletype" value="N" <?php echo sent_check("N",$elements[76]);?> onClick="javascript:showBillDiv(this);">
											<label class="form-check-label">Non-Billable</label>
										</div>
									</div>
								</div>
            ...
            <tr id="jobloc_deduct" >
                ...
                <td>
				<div id="leftflt" class="form-check form-check-inline">
					<input type="checkbox" name="use_jobloc_deduct" class="form-check-input" value='<?php echo $elements[69];?>' <?php echo sent_check($elements[69],'Y');?>>&nbsp;
					<label class="form-check-label">Use Job Location for Applicable Taxes</label>
				</div>
				</td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    ...
                    <span id="leftflt"><input class="form-control w-auto summaryform-formelement" type="text" size=15 name="pfee" value="<?php if($elements[34]=='0.00') echo ""; else  echo $elements[34];?>" maxlength="9"></span>
                    <span id="leftflt">&nbsp;
                    <select name="payfee" class="form-select w-auto summaryform-formelement">
                    <?php
        ...
        <tr>
        <td colspan="2">
            <table border="0" width=100% class="mb-0 align-middle">
                <thead>
			<tr valign="middle">
				<input type="hidden" name="empvalues" id="empvalues">
				<td width="148" class="summaryrow" style="border-bottom: 0px solid #ddd;text-align: left;">Add Person:<?php if(TRICOM_REPORTS=='Y'){ echo $mandatory_madison; }?></td>
				<td class="p-0" colspan="3">
				<div class="d-flex gap-1 align-items-center">
					<span id="leftflt">
						<select name="addemp" class="form-select setcommrolesize" onChange="addCommission('newrow')">
		...
		<input class="summaryform-formelement" type="hidden" size=20 name="payroll" value="" maxlength="20">
		<tr>
			...
			<td>
                            <span id="leftflt">
                            <select class="form-select w-250" name="workcode" id="workcode" style="width:210px" setName='Workers Comp Code' <?php echo $spl_Attribute;?>>
                           
                            <?php
                            getDispWCOptions($elements[39],'',$dd2[3]);	// Displaying Worker Compcode options.
                            ?>
                            </select>&nbsp;
                            <?php
		...
		<span class="summaryform-bold-title" style="visibility: hidden;">
			<div id="leftflt" class="form-check form-check-inline">
				<input class="form-check-input" type="radio" name="tapproval" id="tapproval" value="Manual"  <?php  if($elements[41] != '') echo sent_check("Manual",$elements[41]);else echo'checked';?>>
				<label class="form-check-label">Manual</label>
			</div>
			<div id="leftflt" class="form-check form-check-inline">
				<input class="form-check-input" type="radio" name="tapproval" id="tapproval" value="Online" <?php echo sent_check("Online",$elements[41]);?>>
				<label class="form-check-label">Online</label>
			</div>
		</span>
		<tr>
			<td width="167" class="summaryform-bold-title">PO Number</td>
			<td><span id="leftflt"><input class="form-control w-250" type="text" name="po_num" size=20 value="<?php echo html_tls_specialchars(stripslashes($elements[67]),ENT_QUOTES);?>" maxlength="255"></span></td>
		</tr>
		<tr>
			<td width="167" class="summaryform-bold-title">Department</td>
			<td><span id="leftflt"><input class="form-control w-250" type="text" name="assg_dept" size=20 value="<?php echo html_tls_specialchars(stripslashes($elements[68]),ENT_QUOTES);?>" maxlength="255"></span></td>
		</tr>
		<tr>
			<td width="167" class="summaryform-bold-title">Attention</td><td><input type="text" maxlength="255" value="<?php echo html_tls_specialchars($elements[89],ENT_QUOTES);?>" name="attention" size="20" class="form-control w-250"></td>
		</tr>
		...
		<tr>
			...
			<td width="167" class="summaryform-bold-title">
				Billing Contact
				<div id="dateSelGridDiv" class="weekrule d-inline-block">
					<a href="#" class="tooltip">
						<i class="fa fa-info-circle"></i>
						<span style="width:350px">
							Invoices generated using "Create Invoices by Billing Contact-ASGN" ONLY will be email delivered to this billing contact.
						</span>
					</a>
				</div>
			</td>
		...
		if($mode=="editassign" && PAYROLL_PROCESS_BY == "VERTEX")
		{
			...
			?>
			<div class="form-back accordion-item">
			<table width="100%" border="0" class="crmsummary-edit-table accordion-header" id="crm-joborder-taxsetupDiv-Table">
			<tr class="d-flex align-items-center">
				<td class="crmsummary-content-title w-100">
				...
				<td>
					...
					<div id="crm-joborder-taxsetupDiv-minus"><a onClick="classToggle('taxsetup','minus')" class="form-cl-txtlnk link-success fs-6 fw-bold" href="#crm-joborder-taxsetupDiv-plus"><b>-</b></a></div>
					<div class="form-opcl-btnrightside"><div align="left"></div></div>
					</span>
				</td>
			</tr>
			...
			<div class="jocomp-back accordion-body" id="crm-joborder-taxsetupDiv" name="crm-joborder-taxsetupDiv" <?php echo $style_table;?>>
			<table width="100%" border="0" class="crmsummary-jocomp-table mb-0">		
			<tr>
				<td>
					<?php
		...
		<div class="form-back accordion-item">
		<table width="100%" border="0" class="crmsummary-edit-table accordion-header" id="crm-joborder-hrprocessDiv-Table">
		<tr class="d-flex align-items-center">
			<td width="167" class="crmsummary-content-title w-100">
			...
			<td>
				...
				<div id="crm-joborder-hrprocessDiv-minus"><a onClick="classToggle('hrprocess','minus')" class="form-cl-txtlnk link-success fs-6 fw-bold" href="#crm-joborder-hrprocessDiv-plus"><b>-</b></a></div>
				<div class="form-opcl-btnrightside"><div align="left"></div></div>
				</span>
			</td>
		</tr>
		...
		<div class="jocomp-back accordion-body" id="crm-joborder-hrprocessDiv" name="crm-joborder-hrprocessDiv" <?php echo $style_table;?>>
		<table width="100%" border="0" class="crmsummary-jocomp-table mb-0">		
		<tr valign="middle">
			...
			<td>
				...
				<div class="form-check form-check-inline">
					<input class="form-check-input" name="hpcmphone" type="checkbox" id="hpcmphone" value="phone" <?php echo sent_check($hirepro[0],'phone');?>>
					<label class="form-check-label">Phone</label>
				</div>
				<div class="form-check form-check-inline">
					<input class="form-check-input" name="hpcmmobile" type="checkbox" id="hpcmmobile" value="Mobile" <?php echo sent_check($hirepro[1],'Mobile');?>>
					<label class="form-check-label">Mobile</label>
				</div>
				<div class="form-check form-check-inline">
					<input class="form-check-input" name="hpcmfax" type="checkbox" id="hpcmfax" value="Fax" <?php echo sent_check($hirepro[2],'Fax');?>>
					<label class="form-check-label">Fax</label>
				</div>
				<div class="form-check form-check-inline">
					<input class="form-check-input" name="hpcmemail" type="checkbox" id="hpcmemail" value="Email" <?php echo sent_check($hirepro[3],'Email');?>>
		...
		<tr>
			...
			<td>
				<div class="form-check form-check-inline">
					<input class="form-check-input" name="hrresume" type="checkbox" id="hrresume" value="Resume" <?php echo sent_check($hirepro[4],'Resume');?>>
					<label class="form-check-label">Resume</label>
				</div>
				<div class="form-check form-check-inline">
					<input class="form-check-input" name="pinterview" type="checkbox" id="pinterview" value="Pinterview" <?php echo sent_check($hirepro[5],'Pinterview');?>>
					<label class="form-check-label">Phone Interview</label>
				</div>
				<div class="form-check form-check-inline">
					<span class="d-flex align-items-center">
					<input class="form-check-input" name="interview" type="checkbox" id="interview" value="Interview" <?php echo sent_check($hirepro[6],'Interview');?>>
					<label class="form-check-label">Interview (avg #<input name="hpraverage" type="text" id="hpraverage" value="<?php echo html_tls_specialchars($elements[47],ENT_QUOTES);?>" size=15 maxlength="2" class="form-control form-control-sm">)</label>
					</span>
				</div>
				
				<div class="d-flex align-items-center mt-1">
				<div class="form-check form-check-inline">
					<input class="form-check-input" name="backgr" type="checkbox" id="backgr" value="Bcheck" <?php echo sent_check($hirepro[7],'Bcheck');?>>
					<label class="form-check-label">Background Check</label>
				</div>
				<div class="form-check form-check-inline">
					<input class="form-check-input" name="drug" type="checkbox" id="drug" value="Dscreen" <?php echo sent_check($hirepro[8],'Dscreen');?>>
					<label class="form-check-label">Drug Screen</label>
				</div>
				<div class="form-check form-check-inline">
					<input class="form-check-input" name="physical" type="checkbox" id="physical" value="Physical" <?php echo sent_check($hirepro[9],'Physical');?>>
					<label class="form-check-label">Physical</label>
				</div>
				<div class="form-check form-check-inline">
					<input class="form-check-input" name="govt" type="checkbox" id="govt" value="Gclearance" <?php echo sent_check($hirepro[10],'Gclearance');?>>
				...
				<div class="form-check form-check-inline">
					<span class="d-flex align-items-center">
					<input class="form-check-input" name="addcheck" type="checkbox" id="addcheck" value="Addinfo" <?php echo sent_check($hirepro[11],'Addinfo');?>>
					<label class="form-check-label">
						Additional Info
						<input name="addinfo" type=text class="form-control form-control-sm" id="addinfo" value="<?php echo html_tls_specialchars($elements[46],ENT_QUOTES);?>"  size=50 maxsize=100>
					</label>
					</span>
				</div>
				</div>
		...
		<div class="form-back pb-0">
		<table width="100%" border="0" class="crmsummary-edit-table align-middle mb-0">
		<tr>
			<td width="167" valign="middle" class="crmsummary-content-title">Notes</td>
		...
		?>
		<div class="form-back accordion-item">
			<table width="100%" border="0" class="crmsummary-edit-table accordion-header" id="crm-joborder-historicalRatesDiv-Table">
			<tr class="d-flex align-items-center">
				<td width="" class="crmsummary-content-title w-100">
				...
				<td>
					...
					<div id="crm-joborder-historicalRatesDiv-minus"><a onClick="classToggle('historicalRates','minus')" class="form-cl-txtlnk link-success fs-6 fw-bold" href="#crm-joborder-historicalRatesDiv-plus"><b>-</b></a></div>
					<div class="form-opcl-btnrightside"><div align="left"></div></div>
					</span>
				</td>
			</tr>
		    ...
		    <div class="jocomp-back accordion-body" id="crm-joborder-historicalRatesDiv" name="crm-joborder-historicalRatesDiv" <?php echo $style_table;?>>
			    <!--<table width="100%" border="0" class="crmsummary-jocomp-table">		
				<tr>
			    ...
			    <table width="100%" class="mb-0" cellspacing="0" cellpadding="5" border="0" align="left" style="padding-left:0px;">
				<tbody>
		...
		{
			?>
			<div class="form-back card-body pb-0">
			<table width="100%" border="0" class="crmsummary-edit-table accordion-header">
			<tr>
...
<div id="reassignshiftdialog" title="NoShow/Re-Assign" style="display:none;">						
	...
	<div class="reassignbtnNew"><input type="button" class="btn btn-dark" name="reassign" id="reassign" value="Re-Assign" onClick="reassignProcess()"/></div>
</div>
<!-- END -->
...
<div id="cancelshiftdialog" title="Cancel Shift(s)" style="display:none;">						
	...
	<div class="reassignbtnNew"><input type="button" class="btn btn-dark" name="cancelshift" id="cancelshift" value="Cancel Shift" onClick="cancelShiftProcess()"/></div>
</div>
<!-- END -->
